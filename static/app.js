...
function loadStrategy(strategy) {
    ...
    if (strategy.inputs) {
        html += `<div class="glass-panel p-6 rounded-2xl space-y-4">
            <h4 class="text-sm uppercase tracking-wider text-slate-500 font-bold mb-4">Input Required</h4>
...
async function executeAction(actionName) {
    ...
    try {
        ...
        resultArea.innerHTML = `
            <h5 class="text-sm font-bold uppercase tracking-wide mb-2 ${result.status === 'warning' ? 'text-brand-warning' : 'text-brand-accent'}">
                ${result.message}
            </h5>
            <p class="text-white text-lg">${result.data}</p>
        `;
